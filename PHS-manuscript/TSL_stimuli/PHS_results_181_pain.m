...
rgb_cold = blue5{2}; 
rgb_coldp = blue5{2};
rgb_cold_pain = blue5{5};
rgb_cold_pain_bpress = 'k';
gray = [17 17 17];

baseline = 32;
%temperatures = [33.26 30.16 33.58 29.88 33.66 30.31];
mean_temperatures = [32 14.00 38 14.00 44 14.00];
%sd_temperatures = [0 9.16 0 9.71 0 9.96];
int = 0.1;
xint = 20; % 10s baseline
...
%%
animation_fname = 'PHS_newTSL_pain.gif';
[A,map] = rgb2ind(frame2im(getframe),256);
imwrite(A,map,animation_fname,'LoopCount',65535,'DelayTime',0.01);

% Legend
hold on
plot(10, 48, 'Color', [0.5 0.5 0.5], 'Marker', 'd', 'MarkerFaceColor', [0.5 0.5 0.5], 'MarkerSize', 10); % diamond
text(16, 48, 'Auditory cue', 'FontSize',12)
plot(10, 45, 'Color', rgb_cold_pain_bpress, 'Marker', 'o', 'MarkerSize', 10);
plot(10, 45, 'Color', rgb_cold_pain_bpress, 'Marker', 'o', 'MarkerFaceColor', rgb_cold_pain, 'MarkerSize', 8);
text(16, 45, 'Button Press: Pain', 'FontSize',12)
...
plot_animatedline(t1, t2, int, rgb_warm1, xint, beep, button_press, rgb_warmp1, et, animation_fname)
...
% plot_animatedline(t1, t2, int, rgb_warm1, xint, beep, button_press, rgb_warmp1, et)
...
plot_animatedline(t1, t2, int, rgb_cold_pain, xint, beep, button_press, rgb_cold_pain_bpress, et, animation_fname)
xint = xint + abs(t1-t2); % determine x-axis coordinate where to plot the next segment and draw text 
% xT_m = xint - 6; yT_m = mean_temperatures(2) - 2; % T stands for Threshold, m for mean, s for std
% xT_s = xint - 5.5; yT_s = mean_temperatures(2) - 3.6;
...
plot_animatedline(t1, t2, int, rgb_warm2, xint, beep, button_press, rgb_warmp2, et, animation_fname)
xint = xint + abs(t1-t2); % determine x-axis coordinate where to plot the next segment and draw text
xbase = xint - 4.5;
...
plot_animatedline(t1, t2, int, rgb_cold_pain, xint, beep, button_press, rgb_cold_pain_bpress, et, animation_fname)
xint = xint + abs(t1-t2); % determine x-axis coordinate where to plot the next segment and draw text
% xT_m = xint - 6; yT_m = mean_temperatures(4) - 2; % T stands for Threshold, m for mean, s for std
% xT_s = xint - 5.5; yT_s = mean_temperatures(4) - 3.6;
...
plot_animatedline(t1, t2, int, rgb_warm3, xint, beep, button_press, rgb_warmp3, et, animation_fname)
xint = xint + abs(t1-t2); % determine x-axis coordinate where to plot the next segment and draw text
xbase = xint - 4.5;
...
plot_animatedline(t1, t2, int, rgb_cold_pain, xint, beep, button_press, rgb_cold_pain_bpress, et, animation_fname)
xint = xint + abs(t1-t2); % determine x-axis coordinate where to plot the next segment and draw text
% xT_m = xint - 6; yT_m = mean_temperatures(6) - 2; % T stands for Threshold, m for mean, s for std
% xT_s = xint - 5.5; yT_s = mean_temperatures(6) - 3.6;
