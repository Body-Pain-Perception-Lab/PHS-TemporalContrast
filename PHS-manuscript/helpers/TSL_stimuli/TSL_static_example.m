...
temperatures = [38.31 29.60 38.27 26.69 39.29 29.30];
int = 0.1;
...
% baseline 
xint = 0; % 5s baseline
t1 = baseline; t2 = baseline; beep = 1; button_press = 0;
plot_animatedline(t1, t2, int, reds5{1}, xint, beep, button_press, reds5{1}, et, animation_fname)

% segment 1: WDT 1
% xint = x0;
t1 = baseline; t2 = temperatures(1);  beep = 0; button_press = 1;
...
text(xint-1,temperatures(2) - 1.5,num2str(temperatures(2)), 'FontSize',12); % add a text (temperature in C) to the plot

% segment 4: return to baseline
t1 = temperatures(2); t2 = baseline; beep = 1; button_press = 0;
plot_animatedline(t1, t2, int, rgb_cold, xint, beep, button_press, rgb_coldp, et, animation_fname)
xint = xint + abs(t1-t2);  % determine x-axis coordinate where to plot the next segment

% segment 5: WDT 2
t1 = baseline; t2 = temperatures(3); beep = 0; button_press = 1;
plot_animatedline(t1, t2, int, rgb_warm, xint, beep, button_press, rgb_warmp, et, animation_fname)
xint = xint + abs(t1-t2); % determine x-axis coordinate where to plot the next segment and draw text
...
plot_animatedline(t1, t2, int, rgb_cold, xint, beep, button_press, rgb_coldp, et, animation_fname)
xint = xint + abs(t1-t2); % determine x-axis coordinate where to plot the next segment and draw text
text(xint-1,temperatures(4) - 1.5,num2str(temperatures(4)), 'FontSize',12);

% segment 8: return to baseline
t1 = temperatures(4); t2 = baseline; beep = 1; button_press = 0;
plot_animatedline(t1, t2, int, rgb_cold, xint, beep, button_press, rgb_coldp, et, animation_fname)
xint = xint + abs(t1-t2); % determine x-axis coordinate where to plot the next segment

% segment 9: WDT 3
t1 = baseline; t2 = temperatures(5);  beep = 0; button_press = 1;
plot_animatedline(t1, t2, int, rgb_warm, xint, beep, button_press, rgb_warmp, et, animation_fname)
xint = xint + abs(t1-t2); % determine x-axis coordinate where to plot the next segment and draw text
...
plot_animatedline(t1, t2, int, rgb_cold, xint, beep, button_press, rgb_coldp, et, animation_fname)
xint = xint + abs(t1-t2); % determine x-axis coordinate where to plot the next segment and draw text
text(xint-1,temperatures(6) - 1.5,num2str(temperatures(6)), 'FontSize',12);
...
function plot_animatedline(t1,t2,int, color, x0, beep, button_press, bp_color, et, animation_fname)
    ...
    set(gca, 'XLim', [0 round(et)], 'YLim', [20 45], 'FontSize',12) % X-axis and Y-axis limits
...
%     title('Thermal Sensory Limen (TSL)', 'FontSize',14)
